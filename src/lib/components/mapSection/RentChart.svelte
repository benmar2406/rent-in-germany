<script>
	...
	$: y1 = d3.scaleLinear().domain([0, 140]).range([innerHeight, 0]);

	$: y2 = d3.scaleLinear().domain([0, 140]).range([innerHeight, 0]);
...
<figure class="charts-container" bind:this={chartContainer}>
	...
	<div
		class="chart-wrapper"
		role="group"
		aria-label="Liniendiagramm, dass die Entwicklung und das Auseinanderklaffen zwischen Löhnen und Mieten zeigt"
	>
		<svg bind:this={svg} {width} {height}>
			<g transform={`translate(${marginLeft},${marginTop})`}>
				...
				<text
					...
					dy="-1em"
					fill="#ca3f2d"
					text-anchor="middle">Mietpreisindex</text
				>
				...
				<text
					...
					dy="-1em"
					fill="#2db8ca"
					text-anchor="middle">Reallohnindex (bis 2019)</text
				>
			</g>
		</svg>
        <p class="source">
